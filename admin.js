...
btnOpen.addEventListener('click', async () => {
  ...
  setMsg('開始中…');
  try {
    await patchQuizState({ question_number: selectedNum, question_text: selectedText,
      correct_answer: selectedCorrect, is_open: true, phase: 'open' });
...
btnNext.addEventListener('click', async () => {
  ...
  setMsg('待機中に戻しています…');
  try {
    await patchQuizState({ question_number: 0, question_text: '', correct_answer: null,
      is_open: false, phase: 'waiting' });
